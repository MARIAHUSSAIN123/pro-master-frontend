    ...
    try {
      ...
      if (data?.success) {
        if (data.user?.role !== "customer") {
          setError("Ye account customer portal ke liye nahi hai.");
          setLoading(false);
          return;
        }
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0B1220] px-4">
      <div className="w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-slate-400">Customer Portal</p>
        </div>
        ...
        >
          ...
          <p className="mt-6 text-center text-sm text-slate-400">
            Naya account?{" "}
            <Link to="/portal/signup" className="font-medium text-cyan-400 hover:text-cyan-300">
              Sign up
            </Link>
          </p>
        </form>
